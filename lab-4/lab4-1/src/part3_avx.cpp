...
void process2(uint8_t alpha1, uint8_t alpha2, uint8_t *rgb_buffer1, uint8_t *rgb_buffer2)
{
    ...
    // Pointers to R, G, B planes in the input RGB buffers
    uint8_t *r1_plane_in = rgb_buffer1;
    uint8_t *g1_plane_in = r1_plane_in + (static_cast<size_t>(width) * height);
    uint8_t *b1_plane_in = g1_plane_in + (static_cast<size_t>(width) * height);

    uint8_t *r2_plane_in = rgb_buffer2;
    uint8_t *g2_plane_in = r2_plane_in + (static_cast<size_t>(width) * height);
    uint8_t *b2_plane_in = g2_plane_in + (static_cast<size_t>(width) * height);

    // --- AVX2 Setup ---
    __m256 alpha_factor1_ps = _mm256_set1_ps(static_cast<float>(alpha1) / 255.0f);
    __m256 alpha_factor2_ps = _mm256_set1_ps(static_cast<float>(alpha2) / 255.0f);
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; j += 8) { // Process 8 pixels horizontally
            ...
            // 1. Load 8 R, G, B uint8_t values from both buffers
            __m128i r1_vals_epi8 = _mm_loadu_si64(reinterpret_cast<const void*>(r1_plane_in + y_plane_idx));
            __m128i g1_vals_epi8 = _mm_loadu_si64(reinterpret_cast<const void*>(g1_plane_in + y_plane_idx));
            __m128i b1_vals_epi8 = _mm_loadu_si64(reinterpret_cast<const void*>(b1_plane_in + y_plane_idx));

            __m128i r2_vals_epi8 = _mm_loadu_si64(reinterpret_cast<const void*>(r2_plane_in + y_plane_idx));
            __m128i g2_vals_epi8 = _mm_loadu_si64(reinterpret_cast<const void*>(g2_plane_in + y_plane_idx));
            __m128i b2_vals_epi8 = _mm_loadu_si64(reinterpret_cast<const void*>(b2_plane_in + y_plane_idx));

            // 2. Convert uint8_t to float32 (8 values each per buffer)
            __m256i r1_vals_epi32 = _mm256_cvtepu8_epi32(r1_vals_epi8);
            __m256i g1_vals_epi32 = _mm256_cvtepu8_epi32(g1_vals_epi8);
            __m256i b1_vals_epi32 = _mm256_cvtepu8_epi32(b1_vals_epi8);

            __m256i r2_vals_epi32 = _mm256_cvtepu8_epi32(r2_vals_epi8);
            __m256i g2_vals_epi32 = _mm256_cvtepu8_epi32(g2_vals_epi8);
            __m256i b2_vals_epi32 = _mm256_cvtepu8_epi32(b2_vals_epi8);

            __m256 r1_vals_ps = _mm256_cvtepi32_ps(r1_vals_epi32);
            __m256 g1_vals_ps = _mm256_cvtepi32_ps(g1_vals_epi32);
            __m256 b1_vals_ps = _mm256_cvtepi32_ps(b1_vals_epi32);

            __m256 r2_vals_ps = _mm256_cvtepi32_ps(r2_vals_epi32);
            __m256 g2_vals_ps = _mm256_cvtepi32_ps(g2_vals_epi32);
            __m256 b2_vals_ps = _mm256_cvtepi32_ps(b2_vals_epi32);

            // 3. Perform blending: blended_color = color1 * alpha_factor1 + color2 * alpha_factor2
            __m256 r_blended_ps = _mm256_fmadd_ps(r1_vals_ps, alpha_factor1_ps, _mm256_mul_ps(r2_vals_ps, alpha_factor2_ps));
            __m256 g_blended_ps = _mm256_fmadd_ps(g1_vals_ps, alpha_factor1_ps, _mm256_mul_ps(g2_vals_ps, alpha_factor2_ps));
            __m256 b_blended_ps = _mm256_fmadd_ps(b1_vals_ps, alpha_factor1_ps, _mm256_mul_ps(b2_vals_ps, alpha_factor2_ps));

            // 4. Y Calculation (for 8 pixels) using blended values
            __m256 y_calc_ps = y_offset_ps;
            y_calc_ps = _mm256_fmadd_ps(r_blended_ps, y_s1_r_ps, y_calc_ps);
            y_calc_ps = _mm256_fmadd_ps(g_blended_ps, y_s2_g_ps, y_calc_ps);
            y_calc_ps = _mm256_fmadd_ps(b_blended_ps, y_s3_b_ps, y_calc_ps);
            ...
            if (i % 2 == 0) {
                // Select blended R', G', B' for pixels 0, 2, 4, 6 from the current block of 8
                __m128 r_for_uv_ps = _mm256_castps256_ps128(_mm256_permutevar8x32_ps(r_blended_ps, uv_select_indices));
                __m128 g_for_uv_ps = _mm256_castps256_ps128(_mm256_permutevar8x32_ps(g_blended_ps, uv_select_indices));
                __m128 b_for_uv_ps = _mm256_castps256_ps128(_mm256_permutevar8x32_ps(b_blended_ps, uv_select_indices));
